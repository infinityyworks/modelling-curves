...
A script to model the relationship between bivariate data. Currently approximates linear relationships in the form y = mx + c, exponential relationships in the form y = kb<sup>x</sup>, and polynomial relationships in the form y = ax<sup>n</sup>.
## Entering data
...
## Output
The program will propose three possible relationships in the forms y = mx + c, y = kb<sup>x</sup> and y = ax<sup>n</sup>. For each one, it will show the corresponding Pearson's correlation coefficient for:
- The linear regression line between the dependent dataset against the independent dataset (linear relationship)
